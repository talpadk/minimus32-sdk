...
print $hFile '#ifndef MOD_VERTICAL_BYTE_FONT_'.$config{'output'}."\n";
print $hFile '#define MOD_VERTICAL_BYTE_FONT_'.$config{'output'}."\n\n";

print $hFile '#include "vertical_byte_font.h"'."\n\n";

print $hFile 'const vertical_byte_font '.$config{'output'}.";\n\n";
...
for (my $i=$config{'start'}; $i<=$config{'end'}; $i++){
	...
	print $cFile '// ';
	for (my $y=0; $y<$config{'height'}*8; $y++){
...
print $cFile 'const vertical_byte_font '.$config{'output'}."={\n";
